...
void* rsvr_dynamic_array_get(rsvr_dynamic_array_t* array,
                             unsigned int index) {
  if (index >= array->amount) {
    return NULL;
  }

  return (void*)((unsigned char*)array->data +
                 index * array->element_size);
}

void rsvr_dynamic_array_push(rsvr_dynamic_array_t* array,
                             const void* element) {
  unsigned char* destination;

  if (array->amount >= array->capacity) {
    array->capacity = (size_t)ceil(array->capacity *
  ...
  destination = (unsigned char*)array->data +
                array->amount * array->element_size;
